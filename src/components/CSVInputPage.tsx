...
export const CSVInputPage: React.FC<CSVInputPageProps> = ({
  onWordsLoaded,
}) => {
  ...
  const lineCount =
    csvText.trim() === '' ? 0 : csvText.trim().split('\n').length
      ...
      <div className="max-w-2xl w-full fade-in">
        <div className="text-center mb-6 md:mb-8">
          ...
          <p className="text-gray-600 mt-3 md:mt-4 text-base md:text-lg px-4">
            Master your vocabulary with interactive flashcards
          </p>
        </div>
        ...
        <div className="glass rounded-2xl shadow-2xl p-4 sm:p-6 md:p-8 backdrop-blur-xl">
          <div className="mb-4 sm:mb-6">
            ...
            >
              {ICONS.PENCIL} Paste CSV Text{' '}
              <span className="text-purple-600">
                (Max {MAX_WORDS_LIMIT} words)
              </span>
            </label>
            <textarea
              id="csv-input"
            ...
            <div className="flex justify-between items-center mt-2 sm:mt-3">
              <p className="text-xs sm:text-sm text-gray-500">
                <span className="font-medium">Format:</span> word, meaning,
                example (optional)
              </p>
        ...
        <div className="mt-4 sm:mt-6 text-center">
          <div className="inline-block text-left bg-white/50 rounded-lg p-3 sm:p-4 max-w-lg">
            ...
            <div className="space-y-1.5 text-[10px] sm:text-xs text-gray-600">
              <p>
                <span className="font-semibold">1.</span> Paste CSV: word,
                meaning, example / CSV形式で貼り付け（単語、意味、例文）
              </p>
              <p>
                <span className="font-semibold">2.</span> Start quiz (random or
                in order) / クイズを開始（ランダムまたは順番）
              </p>
              <p>
                <span className="font-semibold">3.</span> Show answer and mark
                correct/wrong / 答えを表示して正解/不正解をマーク
              </p>
              <p>
                <span className="font-semibold">4.</span> Review results and
                retry wrong words / 結果確認と間違えた単語の復習
              </p>
              <p>
                <span className="font-semibold">5.</span> View wrong words in
                CSV format / 間違えた単語をCSV形式で確認可能
              </p>
            </div>
          </div>
        </div>
